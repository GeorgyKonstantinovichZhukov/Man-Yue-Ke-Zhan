...
class OrderRoom(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(self, parent, id, title = "OrderRoom", size =(1000, 750))
        panel = wx.Panel(self)
        ...
        wx.StaticText(panel, label = "房间ID", pos = (200,400))
        wx.StaticText(panel, label = "酒店ID", pos = (250,400))
        wx.StaticText(panel, label = "类型", pos = (315,400))
        wx.StaticText(panel, label = "等级", pos = (375,400))
        wx.StaticText(panel, label = "价格", pos = (425,400))
        wx.StaticText(panel, label = "状态", pos = (485,400))
        ...
        self.bt1 = wx.Button(panel, label = "查询空闲房间", pos = (200,360))
        self.bt2 = wx.Button(panel, label = "预定房间", pos = (300,360))
        self.bt3 = wx.Button(panel, label = "返回主界面", pos = (400,360))
        self.bt1.Bind(wx.EVT_BUTTON, self.bt1_f)
        self.bt2.Bind(wx.EVT_BUTTON, self.bt2_f)
        self.bt3.Bind(wx.EVT_BUTTON, self.bt3_f)

        self.Text = wx.TextCtrl(panel, pos = (200,425), size = (550, 250),style =  wx.TE_MULTILINE)
    ...
    def bt2_f(self, event):
        ...
        if Cus_ID == "" or name == "" or Tel == "" or Room_ID == "" or Hotle_ID == "":
            ...
            inf2 = cur.fetchall()
            cur.execute("SELECT 住房订单ID FROM HOrder")
            inf3 = cur.fetchall()
            Sum = 0
            for i in inf3:
                Sum = Sum+1
            ...
            if len(inf2) != 0:
                if len(inf) == 0:
                    ...
                    cur.execute("SELECT 价格 FROM Room WHERE (房间ID, 酒店ID) = (?, ?)", (Room_ID, Hotle_ID))
                    cost = cur.fetchall()
                    cur.execute("INSERT INTO HOrder(住房订单ID, 房间ID, 酒店ID, 客户ID, 日期, 总消费) VALUES(?, ?, ?, ?, ?, ?)", (Sum, Room_ID, Hotle_ID, Cus_ID, State, cost[0][0]))
