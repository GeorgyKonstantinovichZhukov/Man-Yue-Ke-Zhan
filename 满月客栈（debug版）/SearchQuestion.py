...
class SearchRoomorder(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(self, parent, id, title = "查询客户反馈", size =(700, 700))
        panel = wx.Panel(self)
        ...
        self.bt1 = wx.Button(panel, label = "查询所有问题", pos = (180,160))
        self.bt2 = wx.Button(panel, label = "删除", pos = (310,160))
        self.bt1.Bind(wx.EVT_BUTTON, self.bt1_f)
        self.bt2.Bind(wx.EVT_BUTTON, self.bt2_f)

        wx.StaticText(panel, label = "客户ID", pos = (50,200))
        wx.StaticText(panel, label = "名字", pos = (110,200))
        wx.StaticText(panel, label = "电话", pos = (190,200))
        wx.StaticText(panel, label = "酒店ID", pos = (280,200))
        wx.StaticText(panel, label = "房间ID", pos = (340,200))
        wx.StaticText(panel, label = "问题内容", pos = (390,200))

        self.Text = wx.TextCtrl(panel, pos = (50,220), size = (600, 400),style =  wx.TE_MULTILINE)
    ...
    def bt2_f(self, event):
        ...
        if C_ID == "" or H_ID == "" or R_ID == "":
            ...
            cur.execute("SELECT * FROM Question WHERE 房间ID = ? AND 酒店ID = ? AND 客户ID = ?",(R_ID,H_ID,C_ID))
            inf = cur.fetchall()
            if inf == []:
                message = "信息不存在"
                wx.MessageBox(message)
